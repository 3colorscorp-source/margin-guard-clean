...
10. **Estimate PDFs are no longer emailed as public storage URLs.** `send-quote-zapier.js` still uploads to `estimate-pdfs` with `public: true` (compatible phase; production bucket is not flipped). New `pdf_url` values are `/.netlify/functions/get-estimate-pdf?token=&path=`. `get-estimate-pdf` requires the quote public token or an Owner/Seller session, contrasts the object tenant segment with that authority, rejects traversal, and 302s to a 60s signed URL. Path guessing without a token is 401. `tenant-logos` stays public. `contract-signed-pdfs` stays private. Frozen in `scripts/test-core-estimate-pdf-access.js`. The CORE SECURITY TEST path was used only as a fixture; customer PDFs were not fetched.
