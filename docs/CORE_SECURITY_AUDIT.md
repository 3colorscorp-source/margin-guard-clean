...
1. **Estimates Zapier outbound HMAC Phase 1 + Catch Hook verifier contract.** `send-quote-zapier.js` and `resend-tenant-quote.js` sign with Invoice Hub v1 (`timestamp.nonce.JSON`) when `ZAPIER_WEBHOOK_SECRET` is set (`ESTIMATES_HMAC_PHASE1_COMPATIBILITY_MODE`). Missing secret still sends unsigned. After signing, the exact canonical string is attached as `zapier_signed_payload` so a Zapier Code Step can verify without reconstructing Catch Hook JSON. Catch Hook does not keep the raw body or `X-MG-*` headers (`docs/CH-013A21Z-ZAPIER-CONTRACT-EMAIL.md`). Phase 2 fail-closed on Netlify is a separate PR.
