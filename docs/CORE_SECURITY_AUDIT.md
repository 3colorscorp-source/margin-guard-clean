...
1. **Estimates Zapier outbound HMAC Phase 1 + Catch Hook verifier + server verifier.** `send-quote-zapier.js` and `resend-tenant-quote.js` sign with Invoice Hub v1 (`timestamp.nonce.JSON`) when `ZAPIER_WEBHOOK_SECRET` is set (`ESTIMATES_HMAC_PHASE1_COMPATIBILITY_MODE`). Missing secret still sends unsigned. After signing, the exact canonical string is attached as `zapier_signed_payload`. `verify-estimates-zapier-hmac.js` verifies that string server-side from Netlify env `ZAPIER_WEBHOOK_SECRET` only (Zapier does not store the secret). Authorized outputs are `signature_valid`, `final_to`, `final_additional_recipients`, `final_subject`, and `final_body`, taken only from the signed JSON. Flattened Catch Hook `client_email` / `additional_recipients` cannot redirect a signed quote. Catch Hook does not keep the raw body or `X-MG-*` headers (`docs/CH-013A21Z-ZAPIER-CONTRACT-EMAIL.md`). Sender fail-closed is a separate PR.
