...
Thank you.`
  ...
  const HUB_PAYMENT_REMINDER_ENDPOINT = "/.netlify/functions/send-invoice-payment-reminder";

  function hubRowValidCustomerEmail(row) {
    const email = String(row?.customerEmail || row?.project?.clientEmail || "").trim();
    return email.includes("@") && email.includes(".") && email.length >= 5 && email.length < 320;
  }

  function hubRowPublicToken(row) {
    return nonEmptyString(row?.project?.invoice?.publicToken);
  }

  function hubRowServerInvoiceUuid(row) {
    return nonEmptyString(row?.serverInvoiceId);
  }

  function hubRowIsArchivedInvoice(row) {
    return String(row?.hubInvoiceRawStatus || row?.status || "")
      .trim()
  ...
  function hubRowIsPaidForReminder(row) {
    ...
    if (bal <= 0.005) return true;
    const amt = finiteNumber(row?.amount, 0);
    const paid = finiteNumber(row?.depositApplied, 0) + finiteNumber(row?.receivedApplied, 0);
    if (amt > 0 && paid + 0.005 >= amt) return true;
    return false;
  }
  ...
  function getHubOverflowMenuState(row) {
    ...
    return {
      ...
      canCopyLink: base.canOpenPublic,
      canResendInvoice,
      canSendInvoice,
  ...
  function formatHubPaymentReminderError(data, status) {
    ...
    const map = {
      ...
      reminder_webhook_not_configured: "Payment reminder service is not configured.",
      zapier_error: "Reminder could not be delivered. Try again later.",
      webhook_unreachable: "Could not reach the reminder service."
    };
    return map[reason] || msg || `Payment reminder failed (HTTP ${status}).`;
  }

  function hubReminderBusinessName(row, settings) {
    const invoice = getProjectInvoiceState(row?.project);
    return (
      nonEmptyString(invoice.businessName, row?.project?.business_name, settings?.bizName, settings?.businessName) ||
      "our team"
    );
  }

  function buildHubPaymentReminderCopy(row, settings) {
  ...
  function hubPaymentReminderSetFeedback(message, tone) {
    const el = $("hubPaymentReminderFeedback");
    if (!el) return;
    if (!message) {
      el.style.display = "none";
      el.textContent = "";
  ...
  function hubPaymentReminderCloseModal() {
    const modal = $("hubPaymentReminderModal");
    if (modal) modal.setAttribute("aria-hidden", "true");
    hubPaymentReminderState = { row: null, sending: false };
    hubPaymentReminderSetFeedback("");
    const sendBtn = $("btnHubPaymentReminderSend");
    if (sendBtn) {
  ...
  function openHubPaymentReminderModal(row, settings) {
    ...
    const copy = buildHubPaymentReminderCopy(row, settings || loadSettings());
    hubPaymentReminderState = { row, sending: false };
    hubPaymentReminderSetFeedback("");
    if ($("hubPaymentReminderSubtitle")) {
      $("hubPaymentReminderSubtitle").textContent = `This message will be sent to ${copy.customerEmail}.`;
    }
    if ($("hubPaymentReminderSubject")) $("hubPaymentReminderSubject").textContent = copy.subject;
    if ($("hubPaymentReminderMessage")) $("hubPaymentReminderMessage").textContent = copy.messageText;
    const sendBtn = $("btnHubPaymentReminderSend");
    if (sendBtn) {
      sendBtn.disabled = false;
      sendBtn.textContent = "Send reminder";
    }
    $("hubPaymentReminderModal").setAttribute("aria-hidden", "false");
    return true;
  }

  async function executeHubPaymentReminderSend(row) {
    const iid = hubRowServerInvoiceUuid(row);
    if (!MG_SERVER_INVOICE_UUID_RE.test(iid)) {
      setHubFeedback("Payment reminder is only available for server-backed invoices.", "warn");
      return { ok: false };
    }
    const menuState = getHubOverflowMenuState(row);
    if (!menuState.canSendPaymentReminder) {
    ...
    const email = String(row?.customerEmail || row?.project?.clientEmail || "").trim();
    try {
      const res = await fetch(HUB_PAYMENT_REMINDER_ENDPOINT, {
        method: "POST",
        credentials: "same-origin",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({ invoice_id: iid })
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || data.ok !== true) {
        const msg = formatHubPaymentReminderError(data, res.status);
        hubPaymentReminderSetFeedback(msg, "err");
        setHubFeedback(msg, "err");
        return { ok: false };
      }
      if (data.last_reminder_at) {
        row.hubInvoiceLastReminderAt = data.last_reminder_at;
      }
      await refreshHubServerInvoicesCacheQuietly();
      const when = data.last_reminder_at ? formatDisplayDate(data.last_reminder_at) : "just now";
      hubPaymentReminderCloseModal();
      setHubFeedback(`Payment reminder sent to ${email} (${when}).`, "ok");
      return { ok: true, last_reminder_at: data.last_reminder_at };
    } catch (err) {
      const msg = String(err?.message || err || "Network error sending reminder.");
      hubPaymentReminderSetFeedback(msg, "err");
      setHubFeedback(msg, "err");
      return { ok: false };
    }
  ...
  async function sendHubPaymentReminder(row, settings) {
    if (!openHubPaymentReminderModal(row, settings)) {
      return { ok: false };
    }
    return { ok: false, preview: true };
  }

  function closeHubDrawerActionsMenu() {
  ...
  function renderHubDrawerActionsMenu(row) {
    const menu = $("hubDrawerActionsMenu");
    if (!menu || !row) return;
    const menuState = getHubOverflowMenuState(row);
    const sentAt = nonEmptyString(row?.hubInvoiceSentAt);
    ...
    const item = (action, label, enabled) => {
    ...
    menu.innerHTML = [
      ...
      section(
        "Links",
        [
          item("open-public", "Open public link", menuState.canOpenPublic),
          item("copy-link", "Copy public link", menuState.canCopyLink),
          item("share", "Share", menuState.canCopyLink)
        ].join("")
      ),
      section(
        "Document",
        [
          item("download-pdf", "Download PDF", menuState.canDownloadPdf),
          item("print", "Print", menuState.canDownloadPdf)
        ].join("")
      ),
      section(
        "Manage",
  ...
  async function hubDrawerSharePublicLink(row) {
    const url = hubRowPublicUrl(row);
    if (!url) {
      setHubFeedback("No public link to share.", "warn");
      return;
    }
    const title = hubRowInvoiceDisplayLabel(row) || row?.title || "Invoice";
    if (navigator.share) {
      try {
        await navigator.share({ title, text: `Invoice: ${title}`, url });
        setHubFeedback("Share opened.", "ok");
        return;
      } catch (err) {
    ...
    try {
      ...
      setHubFeedback("Public invoice link copied (share unavailable).", "ok");
    } catch (_e) {
      setHubFeedback("Could not share or copy link.", "err");
    }
  }

  function hubDrawerPrintInvoice(row, settings) {
    const url = hubRowPublicUrl(row);
    if (row?.hubRowSource === "server_invoice" && url) {
    ...
    if (!menuStateGuardDownload(row)) return;
    void exportInvoicePdf("hub", row.project, row.report, settings, getProjectInvoiceState(row.project));
  }

  function menuStateGuardDownload(row) {
    const menuState = getHubOverflowMenuState(row);
    if (menuState.canDownloadPdf) return true;
    setHubFeedback("Download is not available for this invoice.", "warn");
    return false;
  }
  ...
  function hubDrawerDownloadPdf(row, settings) {
    if (row?.hubRowSource === "server_invoice") {
      const url = hubRowPublicUrl(row);
      if (!url) {
        setHubFeedback("Publish the invoice first to get a public link.", "warn");
        return;
      }
  ...
  function hubRowPublicUrl(row) {
    const invoice = getProjectInvoiceState(row?.project);
    const token = hubRowPublicToken(row) || nonEmptyString(invoice.publicToken);
    const url = nonEmptyString(invoice.publicUrl) || (token ? `/invoice-public.html?token=${encodeURIComponent(token)}` : "");
    if (!url) return "";
...
window.renderSupervisor = renderSupervisor;
  ...
  const HUB_PROJECT_PAYMENT_STAGE_LABELS = [
    "Start Payment",
    "Progress Payment",
    "Final Payment",
    "Remaining Balance",
  ...
  const HUB_PROJECT_PAYMENT_STAGE_SET = new Set(
    HUB_PROJECT_PAYMENT_STAGE_LABELS.map((s) => s.toLowerCase())
  );
  const HUB_FULL_REMAINING_STAGE_SET = new Set(["remaining balance", "final payment"]);
  ...
  function buildHubRemainingBalanceInvoiceMessage(row, settings) {
    ...
    const businessName = hubReminderBusinessName(row, settings);
    const stage = String($("hubRbPaymentStage")?.value || "Progress Payment").trim() || "Progress Payment";
    return (
      `Hi ${customerName}, here is a ${stage.toLowerCase()} invoice for ${projectName}. ` +
      "Please review the amount due and let us know if you have any questions. " +
  ...
  function formatHubRemainingBalanceInvoiceError(data, status) {
    ...
    const map = {
      ...
      insert_unique_violation:
        "Could not create the project payment draft invoice. Please refresh and try again.",
      insert_failed: "Could not create the project payment draft invoice. Please refresh and try again.",
    ...
    if (/supabase|constraint|duplicate key|violates unique|23505/i.test(msg)) {
      return "Could not create the project payment draft invoice. Please refresh and try again.";
    }
    return msg || "Could not create the project payment draft invoice. Please refresh and try again.";
  }
  ...
  async function openHubRemainingBalanceInvoiceModal(row, settings) {
    ...
    if (!menuState.canCreateRemainingBalanceInvoice) {
      setHubFeedback("Project payment invoice cannot be created for this project right now.", "warn");
      return false;
    }
    hubRemainingBalanceInvoiceSetFeedback("");
  ...
  function hubRemainingBalanceInvoiceSelectedLabel() {
    const raw = String($("hubRbPaymentStage")?.value || "").trim();
    const hit = HUB_PROJECT_PAYMENT_STAGE_LABELS.find((l) => l.toLowerCase() === raw.toLowerCase());
    return hit || "";
  }

  function hubRemainingBalanceInvoiceSyncPaymentStageDefault() {
    const select = $("hubRbPaymentStage");
    if (!select) return;
    const remaining = finiteNumber(hubRemainingBalanceInvoiceState.remainingBalance, 0);
  ...
  function hubRemainingBalanceInvoiceStageWarning() {
    const label = hubRemainingBalanceInvoiceSelectedLabel();
    if (!HUB_FULL_REMAINING_STAGE_SET.has(label.toLowerCase())) return "";
    const remaining = finiteNumber(hubRemainingBalanceInvoiceState.remainingBalance, 0);
    const amt = hubRemainingBalanceInvoiceSelectedAmount();
    if (!Number.isFinite(amt) || Math.abs(amt - remaining) <= 0.01) return "";
    return "This amount is less than the actual remaining project balance. Consider using Progress Payment instead.";
  }
  ...
  function hubRemainingBalanceInvoiceValidateClientAmount() {
    ...
    if (amt > remaining + 0.001) {
      return {
        ok: false,
        message: `Amount cannot exceed remaining balance (${money(remaining, loadSettings().currency)}).`
      };
    }
    const label = hubRemainingBalanceInvoiceSelectedLabel();
  ...
  async function executeHubRemainingBalanceInvoice(row, options) {
    const iid = hubRowServerInvoiceUuid(row);
    if (!iid || !MG_SERVER_INVOICE_UUID_RE.test(iid)) {
      return { ok: false, message: "No valid server invoice for project payment draft." };
    }
    const menuState = getHubOverflowMenuState(row);
    if (!menuState.canCreateRemainingBalanceInvoice) {
      return { ok: false, message: "Project payment invoice cannot be created for this project right now." };
    }
    ...
    return {
      ...
      message: data.warning ? `${data.message || "Draft created."} ${data.warning}` : data.message,
  ...
  function buildPortfolioRowFromServerInvoiceNorm(norm) {
    ...
    const row = {
      ...
      hubInvoiceLastReminderAt: norm.lastReminderAt || "",
      report: stubReport,
      project: stubProject,
  ...
  async function hubRowRemainingBalanceSendFields(row) {
    ...
    return {
      ...
      invoice_label: hubRowInvoiceLabelNormalized(row) || undefined
    };
  }
  ...
  function hubDrawerRenderLastReminder(row) {
    const el = $("hubDrawerLastReminder");
    const wrap = $("hubDrawerLastReminderWrap");
    const isServer = row?.hubRowSource === "server_invoice";
    if (wrap) wrap.style.display = isServer ? "" : "none";
    if (!el) return;
    const raw = nonEmptyString(row?.hubInvoiceLastReminderAt);
    el.textContent = raw ? formatDisplayDate(raw) : "—";
  }
  ...
  function hubRowInvoiceLabelNormalized(row) {
    const inv = row?.project?.invoice && typeof row.project.invoice === "object" ? row.project.invoice : {};
    return sanitizeInvoiceLabelInput(
      nonEmptyString(row?.hubInvoiceLabel, inv.invoiceLabel, inv.invoice_label)
    );
  }

  /** Project payment stage invoice (label-driven; legacy source marker without Material Cost). */
  function hubRowIsProjectPaymentInvoice(row) {
    if (!row) return false;
    if (hubRowIsMaterialCostInvoice(row)) return false;
    const label = hubRowInvoiceLabelNormalized(row);
    if (label && HUB_PROJECT_PAYMENT_STAGE_SET.has(label.toLowerCase())) return true;
    const inv = row.project?.invoice && typeof row.project.invoice === "object" ? row.project.invoice : {};
  ...
  /** True only when invoice_label is Remaining Balance (not every source-linked draft). */
  function hubRowIsRemainingBalanceInvoice(row) {
    if (!row) return false;
    if (hubRowIsMaterialCostInvoice(row)) return false;
    const label = hubRowInvoiceLabelNormalized(row);
    return label.toLowerCase() === HUB_REMAINING_BALANCE_INVOICE_LABEL.toLowerCase();
  }
  ...
  function hubDrawerNextPaymentPlaceholderText(paid, total, row) {
    ...
    if (na === "Paid in full") {
      ...
      if (hubRowIsProjectPaymentInvoice(row)) {
        return "No further balance — this project payment invoice is settled.";
      }
  ...
  function renderHubDrawerDetails(row, settings, handlers) {
    ...
    renderHubDrawerActionsMenu(row);
    closeHubDrawerActionsMenu();
  ...
  function renderEstimatesHub() {
    ...
    const dispatchHubDrawerAction = async (action) => {
      if (!selectedRow) return;
      ...
      if (action === "share") {
        void hubDrawerSharePublicLink(row);
        return;
      }
      ...
      if (action === "print") {
        hubDrawerPrintInvoice(row, settings);
        return;
      }
      ...
      if (action === "archive") {
        ...
        if (!MG_SERVER_INVOICE_UUID_RE.test(iid)) {
          setHubFeedback("No valid invoice to archive.", "warn");
          return;
        }
        const { ok, data } = await postHubInvoiceArchiveDelete(iid, "archive");
        if (!ok) {
          setHubFeedback(data?.error || "Could not archive invoice.", "err");
          return;
        }
        if (typeof window.__mgHubRefetchServerInvoices === "function") {
          await window.__mgHubRefetchServerInvoices();
    ...
    if ($("hubDrawerActionsMenu") && !window.__MG_HUB_DRAWER_MENU_BOUND__) {
      ...
      menuEl.addEventListener("click", (ev) => {
        ...
        ev.preventDefault();
        ev.stopPropagation();
        const action = String(btn.dataset.hubDrawerAction || "").trim();
        if (!action) return;
        void dispatchHubDrawerAction(action);
      });
    }
    if ($("hubPaymentReminderModal") && !window.__MG_HUB_REMINDER_MODAL_BOUND__) {
      window.__MG_HUB_REMINDER_MODAL_BOUND__ = true;
      if ($("btnHubPaymentReminderClose")) $("btnHubPaymentReminderClose").onclick = hubPaymentReminderCloseModal;
      if ($("btnHubPaymentReminderCancel")) $("btnHubPaymentReminderCancel").onclick = hubPaymentReminderCloseModal;
      if ($("btnHubPaymentReminderSend")) {
        $("btnHubPaymentReminderSend").onclick = () => {
          if (hubPaymentReminderState.sending || !hubPaymentReminderState.row) return;
          const row = hubPaymentReminderState.row;
          hubPaymentReminderState.sending = true;
          const sendBtn = $("btnHubPaymentReminderSend");
          if (sendBtn) {
          ...
          void (async () => {
            const result = await executeHubPaymentReminderSend(row);
            hubPaymentReminderState.sending = false;
            if (result.ok) {
              refresh();
              refreshSelectedRow();
              hubDrawerRenderLastReminder(row);
    ...
    if ($("hubRemainingBalanceInvoiceModal") && !window.__MG_HUB_RB_INVOICE_MODAL_BOUND__) {
      ...
      if ($("hubRbAmountModeRemaining")) {
        $("hubRbAmountModeRemaining").onchange = () => {
          hubRemainingBalanceInvoiceSyncManualWrap();
          hubRemainingBalanceInvoiceSyncPaymentStageDefault();
        };
      }
      if ($("hubRbAmountModeManual")) {
        $("hubRbAmountModeManual").onchange = () => {
          hubRemainingBalanceInvoiceSyncManualWrap();
          hubRemainingBalanceInvoiceSyncPaymentStageDefault();
        };
      }
      ...
      if ($("btnHubRemainingBalanceInvoiceConfirm")) {
        $("btnHubRemainingBalanceInvoiceConfirm").onclick = () => {
          ...
          const stageWarn = hubRemainingBalanceInvoiceStageWarning();
          if (stageWarn) {
          ...
          void (async () => {
            ...
            setHubFeedback(
              result.warning
                ? `Project payment draft created. ${result.warning}`
                : result.message || "Project payment draft invoice created.",
    ...
    if ($("btnHubDrawerSendReminder")) {
      $("btnHubDrawerSendReminder").onclick = () => {
        if (!selectedRow) return;
        void (async () => {
          const result = await sendHubPaymentReminder(selectedRow);
          if (result.ok) {
            refresh();
            refreshSelectedRow();
