...
${val("salesInitials") || "MG"}`;
  ...
  function renderSales() {
    ...
    const refresh = () => {
      ...
      if ($("salesPrimaryMeta")) $("salesPrimaryMeta").textContent = activeUnits > 0 ? `${activeUnits.toFixed(2)} ${settings.pricingMode === "day" ? "dias" : "horas"} · ${money(recommendedPerUnit, settings.currency)} por ${settings.pricingMode === "day" ? "dia" : "hora"}` : "Ingresa horas o dias para calcular el recomendado.";
    ...
    if ($("salesHoursLabel")) $("salesHoursLabel").textContent = settings.pricingMode === "day" ? "Horas calculadas automaticamente" : "Horas estimadas";
    ...
    if ($("salesModeHint")) $("salesModeHint").textContent = settings.pricingMode === "day" ? `La empresa cotiza por dia laboral. Cada dia usa ${Number(settings.hoursPerDay || DEFAULTS.hoursPerDay).toFixed(2)} horas.` : "La empresa cotiza por hora. Los dias se calculan automaticamente.";
    if ($("salesEntryHint")) $("salesEntryHint").textContent = settings.pricingMode === "day" ? "Ingresa los dias estimados. El sistema convierte esos dias a horas internas para evaluar el precio." : "Ingresa las horas estimadas. El sistema calcula los dias equivalentes de forma automatica.";
