...
eq("default required count", defaultManifest.required.length, 6);
...
ok(
  ...
  defaultManifest.required.map((row) => row.id).join(",") ===
    "core-session-security,core-tenant-isolation,core-role-permissions,core-secret-boundaries,core-webhook-security,core-financial-endpoints"
);
