/**
 * CH-011B — Create draft Contract Envelope (Owner/Admin).
 * POST /.netlify/functions/contract-envelope-create
 *
 * No signers, email, PDF, Stripe, invoices, ledger, or Payment Intents.
 */
...
const ALLOWED_BODY_KEYS = new Set(["package_id"]);
...
exports.handler = async (event) => {
  try {
    ...
    const packageId = trimField(body.package_id).toLowerCase();
    ...
    const result = await createDraftEnvelope({
      tenantId: tenant.id,
      packageId,
    ...
    if (!result.ok) {
      return json(result.status || 400, {
        ...
        active_envelope_id: result.active_envelope_id,
    ...
    console.error("contract-envelope-create", err);
    return json(500, {
      ok: false,
      error: "Server error",
      code: "server_error",
