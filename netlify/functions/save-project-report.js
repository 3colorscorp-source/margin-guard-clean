...
const { mapDeviceReportRow } = require("./_lib/supervisor-device-field-dto");
const {
  isOwnerContext,
...
exports.handler = async (event) => {
  try {
    ...
    if (isDevice) {
      ...
      if (!supervisorUserId) {
        return json(403, {
          error: "Supervisor must sign in once before saving reports",
    ...
    let inserted;
    try {
      inserted = await supabaseRequest("tenant_project_reports", {
        method: "POST",
    ...
    if (isDevice) {
      const report = mapDeviceReportRow(row);
      if (!report) {
        return json(500, { error: "Save failed", code: "save_failed" });
      }
